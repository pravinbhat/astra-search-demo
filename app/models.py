...
class LibraryBookUpdate(BaseModel):
    ...
    publication_year: Optional[int] = Field(
        None, description="Year the book was published", ge=1000, le=9999
    )
    ...
    metadata: Optional[dict] = Field(None, description="Book metadata (isbn, language, edition)")
    is_checked_out: Optional[bool] = Field(
        None, description="Whether the book is currently checked out"
    )
    borrower: Optional[str] = Field(None, description="Name of the borrower if checked out")
    due_date: Optional[str] = Field(None, description="Due date if checked out")
    vectorize_text: Optional[str] = Field(
...
class LibraryBookResponse(LibraryBookBase):
    ...
    embedding: Optional[list[float]] = Field(
        None, description="Optional embedding payload if stored in the document"
    )
...
class LibraryBookListResponse(BaseModel):
    ...
    library_books: List[LibraryBookResponse] = Field(
        ..., description="List of library book documents"
    )
...
class LibraryBookSearchRequest(BaseModel):
    ...
    query: Optional[str] = Field(
        None, description="Semantic search query used for AstraDB vector search"
    )
    keywords: Optional[str] = Field(None, description="Lexical search keywords for text matching")
